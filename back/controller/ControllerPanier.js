...
export const createProduitPanier = async (req, res) => {
  try {
    ...
    let newAnnonce = await ProduitPanier.create({
      qteAchat,
      idClient: client._id,
      idAnnonce: [idAnnonce], // Initialize the array with idAnnonce
    });

    // Add idAnnonce to the existing array, if any
    if (client.annonces) {
      client.annonces.push(idAnnonce);
    } else {
      client.annonces = [idAnnonce];
    }
